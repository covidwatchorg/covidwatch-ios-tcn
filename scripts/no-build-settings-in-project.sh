...
if [ $NUMBER_OF_BUILD_SETTINGS != $NUMBER_OF_EMPTY_BUILD_SETTINGS ]; then
  ...
  echo "error: Found ${NUMBER_WITH_SETTINGS} build settings in Xcode project file. Build settings should only be defined in .xcconfig files that are located in the COVIDWatch iOS/Configuration folder."
  exit 1
fi
