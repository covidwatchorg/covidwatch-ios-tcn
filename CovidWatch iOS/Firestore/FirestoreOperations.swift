...
struct FirestoreOperations {
    ...
    static func getOperationsToDownloadSignedReports(
        sinceDate: Date,
        using context: NSManagedObjectContext,
        mergingContexts: [NSManagedObjectContext]? = nil
    ) -> [Operation] {
        let downloadOperation = SignedReportsDownloadOperation(
            sinceDate: sinceDate
        )
        let processingOperation = QuerySnapshotProcessingOperation(
            context: context,
            mergingContexts: mergingContexts
        )
        let passDownloadResultsToProcessing = BlockOperation {
            [unowned downloadOperation, unowned processingOperation] in
