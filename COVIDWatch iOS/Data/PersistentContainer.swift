...
public class PersistentContainer: NSPersistentContainer {
    ...
    public func load(_ completionHandler: @escaping (Error?) -> Void) {
        ...
        let logString: StaticString = "Loading persistent stores background task=%d..."
        os_log(logString, log: PersistentContainer.log, self.backgroundTaskIdentifier?.rawValue ?? 0)
        container.loadPersistentStores(completionHandler: { (_, error) in
            ...
            if let error = error {
                let errorString: StaticString = "Loading persistent stores failed: %@"
                os_log(errorString, log: PersistentContainer.log, type: .error, error as CVarArg)
    ...
    public func delete() {
        ...
        do {
            ...
            let dataPath = applicationDataDirectoryURL.appendingPathComponent(
                "\(PersistentContainer.modelName).sqlite"
            ).path
            if fileManager.fileExists(atPath: dataPath) {
                try fileManager.removeItem(atPath: dataPath)
            }
            let shmPath = applicationDataDirectoryURL.appendingPathComponent(
                "\(PersistentContainer.modelName).sqlite-shm"
            ).path
            if fileManager.fileExists(atPath: shmPath) {
                try fileManager.removeItem(atPath: shmPath)
            }
            let walPath = applicationDataDirectoryURL.appendingPathComponent(
                "\(PersistentContainer.modelName).sqlite-wal"
            ).path
