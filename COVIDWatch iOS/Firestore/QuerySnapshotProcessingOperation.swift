...
class QuerySnapshotProcessingOperation: Operation {
    ...
    private func markLocalContactEvents(
        from queryDocumentSnapshots: [QueryDocumentSnapshot], asPotentiallyInfectious infectious: Bool
    ) {
        ...
        self.context.performAndWait { [weak self] in
            do {
                ...
                os_log(
                    "Marking %d contact event(s) as potentially infectious=%d ...",
                    type: .info, identifiers.count, infectious
                )
                ...
                try identifiers.chunked(into: 300000).forEach { (identifiers) in
                    ...
                    batchUpdateRequest.predicate = NSPredicate(
                        format: "identifier IN %@ AND wasPotentiallyInfectious != %d", identifiers, infectious
                    )
                    batchUpdateRequest.resultType = .updatedObjectIDsResultType
                    batchUpdateRequest.propertiesToUpdate = [
                        "wasPotentiallyInfectious": infectious,
                        // Marking a contact event as not infectious means that it was
                        // deleted from the public contact event list.
                ...
                if !allUpdatedObjectIDs.isEmpty, let mergingContexts = self.mergingContexts {
                    NSManagedObjectContext.mergeChanges(
                        fromRemoteContextSave: [NSUpdatedObjectsKey: allUpdatedObjectIDs], into: mergingContexts
                    )
                }
                os_log(
                    "Marked %d contact event(s) as potentially infectious=%d",
                    type: .info, queryDocumentSnapshots.count, infectious
                )
            } catch {
                os_log(
                    "Marking contact event(s) as potentially infectious=%d failed: %@",
                    type: .error, infectious, error as CVarArg
                )
            }
        }
    }
